...
def run_simulation(db_name, exp_type, exp_files, starting_structure, calcs,
    ...
    try:
        ...
        if run_db['rattle'] is not None or run_db['rattle'] is not False:
...
if __name__ == '__main__':
    exp_dict = {'qmin': 0.0, 'qmax': 25., 'qbin': .1, 'rmin': 1.2,
                'rmax': 15, 'rstep': .01}
    calcs = [
        {'name': 'PDF', 'kwargs': {'conv': 300, 'potential': 'rw'}},
        # {'name': 'FQ', 'kwargs': {'conv': 50, 'potential': 'rw'}},
        {'name': 'Spring', 'kwargs': {'k': 1000, 'rt': 1.3}}
    ]
    run_simulation(
        ...
        calcs,
        {'Simulation type': 'NUTS-HMC', 'Sim args': (.65, 10, .5)},
        exp_dict,
        rattle=(.1, 42),
        comments='C60 with Spring'
    )
