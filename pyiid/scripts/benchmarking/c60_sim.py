...
calc = PDFCalc(gobs=pdf, qmin=0, conv=1., qbin=.1, qmax=qmax, potential='rw')
...
traj = simulate_dynamics(atoms, .5e-1, 100)
...
plt.plot(r[100:1200], pdf[100:1200] - wrap_pdf(traj[min_pe], qmin= 0)[0][100:1200] - 15, '-r', label='start-best')
plt.legend()
plt.show()
# '''